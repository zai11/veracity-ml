...
pub fn _log_cosh_loss<T, U>(y_pred: &Array1<T>, y_actual: &Array1<U>) -> Result<f64, EvaluationMetricError>
where
...
pub fn log_cosh_loss<T, U>(y_pred: &DataVector<T>, y_actual: &DataVector<U>) -> Result<f64, EvaluationMetricError>
where
    T: Into<f64> + Copy + Send + Sync + 'static,
    U: Into<f64> + Copy + Send + Sync + 'static
{
    _log_cosh_loss(&y_pred.to_ndarray().unwrap(), &y_actual.to_ndarray().unwrap())
}