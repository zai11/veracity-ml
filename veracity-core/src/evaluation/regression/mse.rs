...
pub fn _mse<T, U>(y_pred: &Array1<T>, y_actual: &Array1<U>) -> Result<f64, EvaluationMetricError>
where
    ...
    U: Into<f64> + Copy + Send + Sync + 'static,
{
    ...
    Ok(total_squared_error / y_pred.len() as f64)
}

pub fn mse<T, U>(y_pred: &DataVector<T>, y_actual: &DataVector<U>) -> Result<f64, EvaluationMetricError>
where
    T: Into<f64> + Copy + Send + Sync + 'static,
    U: Into<f64> + Copy + Send + Sync + 'static
{
    _mse(&y_pred.to_ndarray().unwrap(), &y_actual.to_ndarray().unwrap())
}