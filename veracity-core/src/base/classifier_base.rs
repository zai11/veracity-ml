...
use super::settings_base::ClassifierSettingsBase;

pub trait Classifier<T: Num + Copy, D: Dimension, U> {
    fn _fit(&mut self, x: ArrayBase<OwnedRepr<T>, D>, y: Array1<U>) -> Result<(), VeracityError>;

    fn fit(&mut self, x: &DataMatrix, y: &DataVector) -> Result<(), VeracityError>;

    fn _predict(&self, x: ArrayBase<OwnedRepr<T>, D>) -> Result<Array1<U>, VeracityError>;

    fn predict(&self, x: &DataMatrix) -> Result<DataVector, VeracityError>;

    fn _predict_proba(&self, x: ArrayBase<OwnedRepr<T>, D>) -> Result<Vec<BTreeMap<U, f64>>, VeracityError>;

    fn predict_proba(&self, x: &DataMatrix) -> Result<Vec<DataMatrix>, VeracityError>;

    fn _score(&self, x: ArrayBase<OwnedRepr<T>, D>, y: Array1<U>) -> Result<f64, VeracityError>;
